...
export default function TeaUpload() {
  const [files, setFiles] = useState([]);
  const [results, setResults] = useState([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState("");

  const softLabelNames = ["Plucked", "Flush", "Mature"];
  const indicesNames = ["NDVI", "GNDVI", "EVI", "CIGREEN", "CIRE", "MSI"];
  ...
  const handleUpload = async () => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:8000/tiff/predict_tea_image",
        formData,
  ...
  return (
    <div className="upload-container">
      <div className="upload-box">
        <div className="upload-headline">Upload Tea Images</div>
        <div className="upload-row">
          <label htmlFor="tea-upload" className="upload-icon">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-6">
              <path strokeLinecap="round" strokeLinejoin="round" d="M3 16.5v2.25A2.25 2.25 0 0 0 5.25 21h13.5A2.25 2.25 0 0 0 21 18.75V16.5m-13.5-9L12 3m0 0 4.5 4.5M12 3v13.5"/>
            </svg>
          </label>
          <input id="tea-upload" type="file" multiple accept=".tif,.tiff" onChange={handleFileChange} style={{ display: "none" }} />
