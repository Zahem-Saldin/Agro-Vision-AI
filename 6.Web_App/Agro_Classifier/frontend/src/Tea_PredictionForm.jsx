...
export default function Tea_PredictionForm() {
  ...
  const phaseNames = ["Plucked", "Flush", "Mature"];
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch("http://localhost:8000/kml/predict_tea", {
        method: "POST",
  ...
  const handleSaveToDb = async () => {
    try {
      ...
      const response = await fetch("http://127.0.0.1:8000/save/predictions_tea", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
    ...
    <div>
      <div className="prediction-grid">
        ...
        <div className="instructions-column">
          <div>
            <h3>How to Use the Tea Growth Prediction Tool</h3>
            <p>
    ...
    Classify
      ...
      {Object.entries(groupedResults).map(([year, yearResults]) => {
        ...
        if (quarters.length > 0) {
          ...
          const phaseCounts = { Plucked: 0, Flush: 0, Mature: 0 };
          quarters.forEach((q) => {
