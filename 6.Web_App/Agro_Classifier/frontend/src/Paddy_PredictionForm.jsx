...
export default function Paddy_PredictionForm() {
  ...
  const phaseNames = ["Sowing", "Vegetative", "Harvest"];
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch("http://localhost:8000/kml/predict_paddy", {
        method: "POST",
  ...
  const handleSaveToDb = async () => {
    try {
      ...
      const response = await fetch("http://127.0.0.1:8000/save/predictions_paddy", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
    ...
    <div>
      <div className="prediction-grid">
        ...
        <div className="instructions-column">
          <div>
            <h3>How to Use the Paddy Growth Prediction Tool</h3>
            <p>
    ...
    Classify
      ...
      {Object.entries(groupedResults).map(([year, yearResults]) => {
        ...
        if (quarters.length > 0) {
          ...
          const phaseCounts = { Sowing: 0, Vegetative: 0, Harvest: 0 };
          quarters.forEach((q) => {
