# Coconut_Train_PatchBased_tfdata_WeightedFocal.py
...
# -------------------- SETTINGS --------------------
input_folder_features = r"C:/Users/Zahem Saldin/Desktop/Coconut_CNN_Input"   # 6-band coconut indices
input_folder_labels = r"C:/Users/Zahem Saldin/Desktop/Coconut_SoftLabels"     # 3-band soft labels: low/medium/high density
...
model = Sequential([
    ...
    Conv2D(3, 1, activation='softmax', padding='same')  # low, medium, high density
])

# -------------------- CUSTOM WEIGHTED FOCAL LOSS --------------------
def weighted_focal_loss(alpha, gamma=2.0):
    """
    alpha: list/array of per-class weights [w0, w1, w2]
    ...
    def loss(y_true, y_pred):
        ...
        weight = alpha * y_true             # apply class weighting
        fl = weight * tf.pow(1 - y_pred, gamma) * ce
        return tf.reduce_mean(tf.reduce_sum(fl, axis=-1))  # average over pixels
    return loss

alpha = [10.0, 1.0, 1.0]  # boost precision for minority class (low density)
loss_fn = weighted_focal_loss(alpha, gamma=2.0)
...
# -------------------- SAVE FINAL MODEL --------------------
model.save("coconut_patch_based_softlabel_cnn_improved_2.keras")
print("✅ Final model saved as 'coconut_patch_based_softlabel_cnn_improved_2.keras'")
...
# -------------------- OPTIONAL: RELOAD LOG AND RE-PLOT --------------------
